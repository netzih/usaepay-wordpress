...
/**
 * ISO 3166-1 alpha-2 to alpha-3 country codes. USAePay documents a
 * three-letter code for billing addresses; most applications store alpha-2.
 */
class Country {
  ...
  /**
   * Alpha-3 back to alpha-2, or '' when unknown.
   */
  public static function alpha2(string $alpha3): string {
    $alpha3 = strtoupper(trim($alpha3));
    if (strlen($alpha3) === 2) {
      return isset(self::ALPHA3[$alpha3]) ? $alpha3 : '';
    }
    $alpha2 = array_search($alpha3, self::ALPHA3, TRUE);
    return $alpha2 === FALSE ? '' : $alpha2;
  }

  /**
   * Accepts an alpha-2 or alpha-3 code; returns the alpha-3 code or '' when
   * unknown, in which case the field is left out of the request.
   */
  public static function alpha3(string $code): string {
    $code = strtoupper(trim($code));
    if (strlen($code) === 3) {
      return in_array($code, self::ALPHA3, TRUE) ? $code : '';
    }
